...
void Game::initWindow() {
  ...
  window.create(sf::VideoMode(3024, 1964), "Ninja Platformer");
...
void Game::loadTextures() {
  ...
  for (int i = 0; i < 12; i++) {
    std::string name = "Run" + std::to_string(i + 1) + ".png";

    sf::Texture playerTexture;

    if (!playerTexture.loadFromFile(
            "/Users/deontaeallen/NinjaPlatformer-2/"
            "resources/"
            "Player/run/" +
            name))  // takes texture from
                    // /Users/deontaeallen/NinjaPlatformer-2/resources
                    // folder
    {
  ...
  if (!heartTexture.loadFromFile(
          "/Users/deontaeallen/NinjaPlatformer-2/resources/"
          "Images/heart-icon.png"))  // takes texture
                                     // from
                                     // /Users/deontaeallen/NinjaPlatformer-2/resources
                                     // folder
  {
  ...
  if (!npcTexture.loadFromFile(
          "/Users/deontaeallen/NinjaPlatformer-2/resources/"
          "Images/Adam.jpg"))  // takes texture from
                               // /Users/deontaeallen/NinjaPlatformer-2/resources
                               // folder
  {
    std::cout << "failed to load image.";
  ...
  textures["NPC"] = npcTextures;
  textures["Block"] = blockTextures;
  textures["Heart"] = heartTextures;
...
void Game::run() {
  ...
  // allocates memory for player since there is no default constructor that
  // allows global variable otherwise
  player =
      new Player(&textures["Player"], 10.0f, 12.0f, 1.0f, 72,
                 120);  // instantiates player. passes player textures, max
                        // x-axis speed, max y-axis speed, and acceleration rate
  ...
  while (window.isOpen()) {
    ...
    switch (state) {
      ...
      case PAUSE_MENU:
        if (!createdState[PAUSE_MENU]) {
          setPauseMenu();                   // set up
          createdState[PAUSE_MENU] = true;  // not inserting false to previous
                                            // state in case we want to go back
        }
        pauseMenu();  // logic
        break;
      case FIRST_LEVEL:
    ...
    if (duration.count() * 1000 < SCREEN_TICKS_PER_FRAME) {
      std::this_thread::sleep_for(
          std::chrono::microseconds((int)(sleep * 1000)));
    }
...
void Game::setFirstLevel() {
  ...
  playerSprite->setPosition(
      windowSize.x / 2 - playerSprite->getGlobalBounds().width / 2,
      windowSize.y / 2 - playerSprite->getGlobalBounds().height * 2 - 200);
  ...
  /*
      entities is a vector of derived objects so it has contain pointers
     otherwise object splicing occurs. allocates memory for enemy otherwise data
     is only copied and the data that the pointer points to is overwritten by
     the program with other variables.
  */
  Enemy *enemy = new Enemy(&textures["Enemy"], 10.0f, 12.0f, 1.0f, 64, 64);
  enemy->getSprite()->setPosition(1000, 400);
  entities.push_back(enemy);

  NPC *npc = new NPC(&textures["NPC"], 10.0f, 12.0f, 1.0f, 64, 64,
                     "Hello, my name is Adam! I love eating raw potatoes while "
                     "watching Korean dramas. My favorite instrument is the "
                     "ocarina I stole from my four year old cousin.");
  npc->getSprite()->setPosition(500, 476);
  entities.push_back(npc);
}

void Game::firstLevel(sf::Event *event) {
  window.clear();
//Triggers when keys are released
  if (event->type == sf::Event::KeyReleased) {
    auto keyCode = event->key.code;
    if (keyCode == sf::Keyboard::D || keyCode == sf::Keyboard::A) {
      player->getSprite()->setTexture(player->getTextures()->at(0));
    }
  }
  ...
  auto textureIndex = static_cast<int>(posX) / 25 % player->getTextures()->size();

  if (player->getRight()) {
    player->getSprite()->setTexture(player->getTextures()->at(textureIndex));
  ...
  for (int i = 0; i < entities.size(); i++) {
    ...
    if (c) {
      if (entity->getClass() == "Enemy") {
        ...
        t.setPosition(entity->getSprite()->getPosition().x,
                      entity->getSprite()->getPosition().y - str.size());
        // t.setString("hello");
        // window.draw(t);
...
std::string Game::mouseCollision() {
  ...
  sf::Vector2f mousePos = window.getView().getCenter() -
                          window.getView().getSize() / 2.0f +
...
void Game::drawUI() {
  ...
  for (int i = 0; i < player->getHealth(); i++) {
    sf::Vector2u textureSize = textures["Heart"].at(0).getSize();
    sf::RectangleShape heart(sf::Vector2f(64, 64));
    heart.setTexture(&textures["Heart"].at(0));
    sf::View view = window.getView();
    heart.setPosition(view.getCenter().x - view.getSize().x / 2 + (i + 1) * 64,
